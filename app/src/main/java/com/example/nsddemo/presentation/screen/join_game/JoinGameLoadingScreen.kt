...
fun JoinGameLoadingScreen(
    ...
    LaunchedEffect(true) {
        viewModel.eventFlow.collectLatest { event ->
            when (event) {
                is UiEvent.NavigateTo -> navController.popBackStackAndNavigateTo(event.destination)
                is UiEvent.ShowSnackBar -> showSnackBar(context.getString(event.messageResId))
                else -> {}
            }
        }
    }
