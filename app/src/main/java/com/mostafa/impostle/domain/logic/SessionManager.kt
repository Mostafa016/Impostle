...
import com.mostafa.impostle.domain.model.Active
...
import com.mostafa.impostle.domain.model.SystemEvent
...
class SessionManager
    ...
    constructor() {
        ...
        private fun performReconnection(
            ...
            val syncRoundData =
                when (val roundData = newData.roundData) {
                    is RoundData.Idle -> {
                        RoundData.Idle
                    }
                    ...
                    is RoundData.QuestionRoundData -> {
                        RoundData.QuestionRoundData(
                            roundPairs =
                                roundData.roundPairs.subList(
                                    0,
                                    roundData.currentPairIndex + 1,
                                ),
            ...
            val syncData =
                newData.copy(
                    localPlayerId = "",
                    imposterId =
                        if (message.playerId == newData.imposterId ||
                            (
                                newPhase
                                    ?: phase
                            ).let { it is GamePhase.GameResults || it is GamePhase.GameReplayChoice || it is GamePhase.GameEnd }
                        ) {
                            newData.imposterId
                        } else {
                            null
                ...
                envelopes =
                    listOf(
                        Envelope.Unicast(
                            ...
                            message =
                                ServerMessage.ReconnectionFullStateSync(
                                    data = syncData,
