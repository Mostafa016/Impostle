...
class CreateGameViewModelTest {
    ...
    @Test
    fun `GIVEN init WHEN session connects THEN sets success and registers player`() = runTest {
        // Arrange & Act
        val viewModel =
            CreateGameViewModel(
    ...
    @Test
    fun `GIVEN init WHEN connection times out THEN sets error state`() = runTest {
        val viewModel =
            CreateGameViewModel(
                mockSessionController,
                mockGameSession,
